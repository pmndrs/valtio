...
## `useSnapshot(state)` without property access does not subscribe
...
it will re-render if `count` changes or its owning subtree is replaced.

Accessing only the object observes changes anywhere inside it:
...
Use `trackKey` to retain whole-container tracking even when its properties
are also accessed:

```js
const tracked = useSnapshot(state)
const obj = trackKey(tracked, 'obj')
obj.count
```

This also observes changes to unread descendants, such as `state.obj.text`.
Ordinary object assignment replaces the child. To update its values in place,
use direct writes or the opt-in `applyChanges` utility.

Calling `useSnapshot(state)` without accessing any properties does not
...
The `tracked` variable returned by `useSnapshot(state)` is tracked for render optimization.
If you pass the `tracked` or some objects in `tracked` to a component with `React.memo`,
...
const ParentComponent = () => {
  ...
  return (
    <div>
      <ChildComponent
        title={tracked.obj.title}
        description={tracked.obj.description}
      />
    </div>
  )
}
...
const ObjectList = React.memo(() => {
  ...
  return tracked.objects.map((object, index) => (
...
## When to use `state` and when to use `tracked` in functional components

- tracked should be used in render function, every other cases state.
...
- deps in useEffect should be used extracting primitive values from tracked. For example: `const { num, string, bool } = tracked.watchObj`.
