...
function CounterList() {
  ...
  return (
    <div>
      <h1>{tracked.title}</h1>
      {/* Touching tracked.items causes the whole list to re-render when any child updates */}
...
function CounterList() {
  ...
  return (
    <div>
      <h1>{tracked.title}</h1>
      {/* Only the length is accessed from tracked, so only updated children re-render */}
      {Array.from({ length: tracked.items.length }, (_, index) => (
        {/* Note that we are passing the proxy object (state) instead of the snapshot (tracked) to the child component */}
...
When you call `tracked.items.map()`, you access every item in the array, causing Valtio to re-render whenever _any_ item changes. By only accessing `tracked.items.length`, Valtio only re-renders when the array length changes. Using `Array.from({ length: n }, (_, i) => ...)` creates an array by index without touching snapshot items, letting you access the proxy directly.
