---
title: 'With React'
description: 'Get started using valtio with react'
---

Using valtio is React is simple and performant.

The library provides two hooks to access the state: 
- From the core: [useSnapshot](/docs/api/basic/useSnapshot).
- From utils: [useProxy](/docs/api/utils/useProxy) 
The first one is used to trigger render on snapshot change in an optimized way and the second one is used to access the state and mutate it.
