...
Among other changes, we will add a Countdown component to display the ticking time for each todo. This component will only rerender when the timeLeft on a single todo changes.
...
Being able to mutate state outside of components is a huge benefit. We can also [`subscribe`](/docs/basic/subscribe) to state changes in module scope. We will leave it to you to try this out. But you could easily persist your todos to local storage inside a `subscribe` defined in module scope as well.
