...
const Todos = () => {
  ...
  return (
    <ul>
      {tracked.todos
        .filter(
          ({ status }) => status === tracked.filter || tracked.filter === 'all',
        )
...
Finally, we need to create, update, and delete our todos. To do so, we simply mutate properties on the store we created, not the tracked. Commonly, these mutations are wrapped up in functions called actions.
...
export const Countdown = ({ index }: { index: number }) => {
  const tracked = useSnapshot(store.todos[index])
  const delta = calcTimeDelta(tracked.timeLeft)
