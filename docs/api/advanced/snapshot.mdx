...
Snapshot data is captured with structural sharing and read-only properties
(see the [Copy on Write](#copy-on-write) section for details).
...
Snapshots maintain the original objects' prototypes, so methods and getters work,
and evaluate against the snapshot's captured state.
...
This also applies to own object getters in v3: `snapshot()` preserves their
accessors without evaluating them. A getter returning a new object therefore
returns a different identity on each access. Exceptions from getters are
thrown when reading them, not when taking the snapshot. Own setters are not
copied to snapshots.
