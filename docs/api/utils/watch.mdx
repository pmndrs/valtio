...
`watch` currently does not implement the usage tracking of `useSnapshot`, so the callback will rerun anytime a `sub`-d proxy (or child proxy) has any of its keys mutated, regardless of the keys accessed within the `callback`'s code.
