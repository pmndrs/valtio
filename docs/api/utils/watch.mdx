...
This utility supports subscribing to multiple proxy objects (unlike `subscribe` which listens to only a single proxy). Proxy objects are subscribed with a `sub` function passed to the callback.
...
watch((add) => {
  // add sessionState to this callback's watched proxies
  add(sessionState)
  const expired = sessionState.expired
  // Or call it inline
  const name = add(userState).user.name
...
```js
watch((add) => {
  const expired = add(sessionState).expired
  const name = add(userState).user.name
  console.log(`${name}'s session is ${expired ? 'expired' : 'valid'}`)
  return () => {
    if (expired) {
