...
const user = proxy({
  ...
  get greetingEn() { return 'Hello ' + user.name },
  // OK - or via `this`
  get greetingFr() { return 'Bonjour ' + this.name },
})
```
```js
const state = proxy({
  user: { // could be nested
    name: 'John',
    // OK - can reference sibling props via the proxy object
    get greetingEn() { return 'Hello ' + state.user.name },
    // OK - or via `this`
    get greetingFr() { return 'Bonjour ' + this.name },
  },
})
...
const state = proxy({
  ...
  greetings: {
    ...
    get greetingFr() { return 'Bonjour ' + this.user.name },
  },
})
```
```js
