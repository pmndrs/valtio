...
describe('detachment dispatch', () => {
  ...
  it('keeps key subscription setup independent of the subtree size', () => {
    ...
    const collect = vi.spyOn(proxyStateMap.get(state.child)!, '4')
  ...
  it('notifies each shared detached target once', () => {
    const shared = proxy({ count: 1 })
    const state = proxy({ child: { a: shared, b: shared } })
    const childListener = vi.fn()
    const sharedListener = vi.fn()
    const stopChild = subscribe(state.child, childListener, true)
    const stopShared = subscribe(shared, sharedListener, true)
