...
const unwrapSnapshotProxy = <T>(value: T): T =>
  (isObject(value) && (snapshotProxyMap.get(value) as T)) || value
...
const createHandlerDefault = <T extends object>(
  ...
  set(target: T, key: string | symbol, value: any, receiver: object) {
    ...
    value = unwrapSnapshotProxy(value)
    ...
    notifyUpdate(
      ...
      extraKeys,
      detached,
      !nextProperty || !('value' in nextProperty),
    )
    return result
  },
})
...
const snapshotProxyMap: WeakMap<object, object> = new WeakMap()
const detachmentVersions: WeakMap<object, number> = new WeakMap()
const accessorVersions: WeakMap<
  object,
...
export function proxy<T extends object>(baseObject: T = {} as T): T {
  ...
  const collectInvalidations = (
    ...
    detachmentVersions.set(baseObject, nextVersion)
    notifications.push(() => {
  ...
  const handler = createHandler<T>(
    ...
    addChildListener,
    removeChildListener,
    (op, key, extraKeys, detached, isAccessor) => {
      const nextVersion = ++versionHolder[0]
      if (isAccessor) {
        // Setters can change refs without touching another tracked key.
        let versions = accessorVersions.get(baseObject)
        if (!versions) {
          accessorVersions.set(baseObject, (versions = new Map()))
        }
...
export function subscribe<T extends object>(
  ...
  const removeListeners = keys
    ? [...new Set(keys.map((key) => normalizeKey(key)))].map((key) =>
        (proxyState as ProxyState)[3](key, listener, true),
      )
...
export function unstable_getInternalStates(): {
  snapshotProxyMap: typeof snapshotProxyMap
  detachmentVersions: typeof detachmentVersions
  accessorVersions: typeof accessorVersions
