...
export function useSnapshot<T extends object>(
  ...
  const subscribeSnapshot = (listener: () => void) => {
    ...
    const createTargetSubscription = (
      ...
      const remove = () => {
        new Set(removes.values()).forEach((stop) => stop())
        removes.clear()
      }
      const notify = () => {
        if (notifyInSync) {
      ...
      const needsKey = (key: Key | undefined, usage: Usage): key is Key =>
        key !== undefined &&
        !!((usage.get(key) || 0) & (TRACK_VALUE | TRACK_HAS | TRACK_DESCRIPTOR))
      const subscribeKeys = (keys: Key[]) =>
        subscribe(target as Record<Key, unknown>, notify, {
          keys,
          sync: true,
          recursive: false,
        })
      const usesWholeTarget = (usage: Usage) =>
        usesContainer(usage) || !!((usage.get(undefined) || 0) & TRACK_KEYS)
      const initialize = (usage: Usage) => {
        if (usesWholeTarget(usage)) {
          removes.set(undefined, subscribe(target, notify, true))
        } else {
          const keys = [...usage.keys()].filter((key): key is Key =>
            needsKey(key, usage),
          )
          if (keys.length) {
            const stop = subscribeKeys(keys)
            keys.forEach((key) => removes.set(key, stop))
          }
        }
      }
      initialize(usage)
      return {
        add: (key, usage) => {
          if (usesWholeTarget(usage) !== removes.has(undefined)) {
            remove()
            initialize(usage)
          } else if (
            !removes.has(undefined) &&
            needsKey(key, usage) &&
            !removes.has(key)
          ) {
            removes.set(key, subscribeKeys([key]))
          }
        },
        remove,
      }
    ...
    const addTargetSubscription = (
      ...
      const subscription = createTargetSubscription(target, usage)
